...
func resolve(hero_state, include_temporary_effects: bool = true) -> RefCounted:
	...
	combat_stats.attack = hero_progression.BASE_ATTACK + effective_strength + hero_state.equipment.get_attack_bonus()
	combat_stats.attack_speed = hero_progression.BASE_ATTACK_SPEED + hero_state.agility * 0.01
	combat_stats.crit_chance = hero_progression.BASE_CRIT_CHANCE + hero_state.agility * 0.01 + hero_state.equipment.get_crit_chance_bonus()
	combat_stats.crit_damage = hero_progression.BASE_CRIT_DAMAGE + hero_state.agility * 0.03 + hero_state.equipment.get_crit_damage_bonus()
