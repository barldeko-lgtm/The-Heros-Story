...
After resurrection the hero appears with `50% MaxHP`, but they recover to full health before the next fight.
...
The hero considers only quests where one ordinary opponent does not exceed **100% of the hero’s current HeroPower**.
...
The hero therefore considers quests only against mobs whose strength does not exceed **100% of the hero’s current strength**.
...
After resurrection the hero has `50% MaxHP`, then recovers in the city at `20% MaxHP` per world tick until fully healed, and only then returns to choosing a new quest.
