...
### 15.11. Current Authored Event Batch — Events 14–15

The final two currently authored Starting Region events deliberately emphasize event inputs that were underrepresented after the first thirteen stories. Before this pair, CON appeared in 8 event Formative openings, STR in 9, and DEX/WIS in 11 each; Cautious and Generous each appeared as an Expressive input in only one event concept. Both new events therefore use a legitimate two-option **CON / STR** highest-primary comparison rather than adding an irrelevant third capability merely for symmetry, and both own one real secondary map objective.

- `medicine_before_sunset` / **«Лекарство до заката»** begins on a non-city road hex 2–4 steps from Starting City. Its temporary field-camp objective is off-road on plains/forest 4–6 steps from the city, 2–4 steps from the encounter and strictly farther from the city. STR spends one preparation tick salvaging the essential medicine, moves Courage `+5`, and delivers a reduced but sufficient shipment whose normal payment is **70 Gold**. CON spends three preparation ticks preserving nearly all usable medicine, moves Greed `+5` toward Generous, and earns a normal **100 Gold** payment. At the destination an Expressive Generous check may instead make the hero spend two additional helping ticks and accept only **20 Gold** for travel expenses. This intentionally makes generosity a costly character expression rather than another disguised reward bonus. The event ends at the remote camp, so the previously interrupted route resumes from that new current hex.
