...
→ future Dungeon / Event systems reuse the same placement/reservation foundation
...
- each current board `QuestOffer` owns one concrete `target_hex` plus the reservation id that protects that hex; quest map placement is real, but legacy `distance_km_min/max` still drives current QuestScore travel estimates and `QuestRunner` travel counters until the separate real-travel migration;
- the accepted quest target stays reserved during selection, legacy outward travel, combat, and between-fight recovery; after the final objective completes and the hero starts returning, the reservation is released; on fatal cancellation it is released immediately; replacement offers acquire a new valid reservation only when their board slot is regenerated;
...
- `MapScreen` is observation-only presentation: it reads runtime hex data, current board QuestOffers, and the live hero position from Simulation; it draws biome textures 1:1 at base zoom, keeps one-pixel black outlines on non-city hexes, omits internal city-hex outlines, draws `town1.png` as a native-size overlay on both seven-hex city clusters, and draws each currently placed QuestOffer with `res://assets/map/activities/quest.png` at its `target_hex`. The supplied 426 × 400 source is kept unchanged and scaled only at draw time to 65 px tall (about 69.2 × 65 px), centered on the target hex. Quest-marker changes are detected for redraw and the sprites scale/pan with map content. The hero sprite still follows only `WorldState.hero_position`, which remains at Starting City during the current legacy abstract quest travel. The screen exposes current coordinates, terrain, region, and permanent semantic tags through the debug hover tooltip and owns only presentation camera state; it must not choose destinations, reserve/release activities, move the hero, calculate travel time, or otherwise modify Simulation;
- opening MapScreen changes visibility only; the existing Simulation continues running;
- ordinary quest-board locations are now integrated; map-backed hero travel, route execution, travel interruption/resumption, events, dungeons, discovery, and hidden-information rules remain intentionally unintegrated.
