...
- empty diary shell;
- rough developer UI;
- a 20 × 15 exact-color PNG-driven hex world foundation where all 300 cells become `HexDefinition` objects with logical coordinates and terrain, including seven Starting City terrain hexes after normalizing the technical hero-start source marker; the map also has runtime route/distance queries, mutable hero position initialized at the Starting City center, and an interactive debug tooltip that shows hovered-hex data;
- automated regression tests and GitHub CI.

Current next major gameplay step:
- continue turning the authored map into explicit per-hex world data in separately approved slices before existing abstract quest/travel mechanics are migrated onto it; the current first per-hex fields are coordinates and terrain.
