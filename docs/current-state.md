...
- empty diary shell;
- rough developer UI;
- a 20 × 15 exact-color PNG-driven hex world foundation where all 300 cells become `HexDefinition` objects with logical coordinates, terrain, and city-region ownership; each city region extends up to seven hex steps from its city center, overlapping candidates belong to the nearer city, four equal-distance boundary hexes are split left/right 2/2, and the resulting current map contains 124 Starting Region hexes, 124 Mid Region hexes, and 52 peripheral hexes with no region; the map also has runtime route/distance queries, mutable hero position initialized at the Starting City center, and an interactive debug tooltip that shows hovered-hex data;
- automated regression tests and GitHub CI.

Current next major gameplay step:
- continue turning the authored map into explicit per-hex world data in separately approved slices before existing abstract quest/travel mechanics are migrated onto it; the current per-hex fields are coordinates, terrain, and region id.

Still missing from the current build:
- diary episodes;
- player-facing quest-guidance selection UI.
- map-backed quest travel time, quest locations, travel interruption/resumption, events, and dungeons on the new map.
