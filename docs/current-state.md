...
Initial creation, ten real minutes, normal close, increased completed-dungeon count and **actually gaining the first specialization** trigger autosaves. Merely fixing the Protector/Slayer target does not count: the specialization milestone fires only after the completed trial is turned in and the hero's granted class becomes the selected specialization. Attaching/loading an already-specialized game initializes the milestone baseline from the restored class, so loading does not immediately rewrite Autosave. Normal-close write failure leaves the game open and paused with an error. Files have integrity checks, verified temporary writes and prior-copy backups; save timestamps increase across both slots. Tests use isolated project `.godot/` directories, never player slots. Focused tests: `test_save_store.gd`, `test_save_ui.gd`, `test_save_confirmations.gd`, `test_save_close_probe.gd`, `test_simulation_snapshot.gd`, `test_snapshot_validation.gd`, `test_snapshot_scenarios.gd`, `test_save_dungeon_milestone.gd`, `test_save_specialization_milestone.gd`.

Snapshot restoration rejects missing/unknown serialized properties, malformed containers/references/RNG and incompatible property types. Typed arrays are reconstructed explicitly, preserving Diary/Log entries and questionnaire answers rather than silently retaining empty defaults. Snapshot schema **v6** preserves the earlier specialization-skill/combat-timer, item-origin and delayed-specialization-grant migrations, then adds the current downtime-statistics dictionary/baseline migration from v5. The Specialization Quest itself adds no duplicate persisted quest flag: its state is derived from the already-saved selected specialization, granted class and persistent specialization-dungeon instance/completion state. The specialization autosave milestone likewise adds no serialized flag: `SaveController` derives the transition from the already-persisted selected/granted class state. Event instances receive a valid construction resource before saved state is hydrated. Regression scenarios compare the complete captured graph immediately after load and after identical dungeon combat, event completion and death-to-recovery continuation. Real dungeon-completion and specialization-gained fixtures each verify a restorable milestone autosave. This is targeted persistence coverage, not full long-run Prototype 0.2 validation.
...
The approved working base-Warrior Skill Level price ladder is keyed to the **hero level at which the purchased rank unlocks**, not to the numerical Skill Level itself. The current hero-level prices are: Level 10 = 500 Gold, 15 = 650, 20 = 850, 25 = 1100, 30 = 1450, 35 = 1900, 40 = 2450, 45 = 3200, 50 = 4150, and 55 = 5400. Therefore Power Strike SL2–SL10 cost 500 / 650 / 850 / 1100 / 1450 / 1900 / 2450 / 3200 / 4150 Gold, while the later-starting Battle Guard SL2–SL10 cost 650 / 850 / 1100 / 1450 / 1900 / 2450 / 3200 / 4150 / 5400 Gold. The first specialization skill is learned at **SL1 for free at Level 25** after the specialization is actually granted. Shield Bash / Crippling Blows then use their own purchased-rank curve starting at **1600 Gold for SL2 at Level 30** and increasing by the same approximately 30% per five-level step with nearest-50 rounding: **1600 / 2100 / 2750 / 3600 / 4700 / 6100 / 7950 / 10350 / 13450 Gold for SL2–SL10**. Autonomous city training is live after the market-sale step and shares one optional-development budget with meaningful equipment after required dungeon preparation is protected. Established Curious buys an affordable unlocked Skill Level before optional equipment; established Conservative buys meaningful affordable equipment first; neutral uses the Warrior default of Skill Level first. The lower-priority category is still allowed on a later tick, and if the preferred category has no valid affordable purchase the same tick falls through to the other category without adding an empty delay. Every successful rank or equipment purchase still consumes its own shopping world tick.
...
The current Crimson Thornplate and Gilded Wyrm families each contain seven standard slots plus Common/Uncommon/Rare Slayer two-handers at ilvl 20 and 25 respectively. Azure Dawnplate covers all twelve equipment slots: five armor pieces with paper-doll overlays plus icon-only jewelry, Belt, sword and shield. The two-handers are present in the matching Arden ordinary-mob and ordinary-dungeon equipment sources, but are filtered out before slot selection unless `hero_class_id == "slayer"`; non-Slayers therefore cannot lose a successful or guaranteed equipment roll to an unusable class item. Conversely, Slayer drop pools exclude one-handed weapons and shields before selection, leaving armor and available class two-handers; this restriction applies to drops only and does not remove ordinary hand items from shop stock.
...
Physical arrival in Арден replaces the active shop with its city-local ilvl 15/20/25 stock. The ilvl 15 Azure Dawnplate band uses all twelve equipment slots and rolls 6 White + 2 Green listings. The ilvl 20 Crimson Thornplate and ilvl 25 Gilded Wyrm bands each contain seven standard slots plus Common/Uncommon Slayer two-handers. `ShopSystem` filters class-restricted definitions before rolling stock, so Warrior/Protector keep the ordinary seven-slot pools while an actually granted Slayer can roll the matching two-hander as the weapon choice without increasing either band's 5 White + 2 Green listing count. Total Arden stock therefore remains **22 equipment listings** when fully stocked.
...
- **first-specialization guidance** — during the Level-20 decision window, one 80-Energy use adds +0.15 to Protector or Slayer and immediately triggers the final current-score comparison;
...
First-specialization guidance is exposed on the developer **Hero screen** inside the specialization debug panel rather than on the God panel, so the player can see the exact current Protector/Slayer weights before spending the one-time influence.
