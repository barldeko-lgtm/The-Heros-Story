...
- an additional ilvl 10 Ironward jewelry set for necklace, earrings, and two mechanically separate ring slots; both rings share the supplied icon, jewelry has no hero paper-doll overlay, and Belt remains unimplemented;
- source-driven ordinary-mob equipment drops keep the same 5% chance and Common/Uncommon/Rare distribution of 70%/25%/5%: the five weakest mobs use the seven-slot ilvl 1 Ironwake source, while Giant Spider (the sixth mob by Power) and all nine stronger mobs use an equal eleven-slot ilvl 10 Ironward source containing five armor slots, sword, shield, necklace, earrings, Ring 1, and Ring 2;
...
- continue the first ordinary-dungeon vertical slice from its working sequential combat + Power retry-readiness + completion-reward loop into Belt/potion preparation/use.

Still missing from the current build:
- diary episodes;
- player-facing quest-guidance selection UI;
- Belt/potion preparation/use and the remaining potion side of dungeon readiness beyond the current first-dungeon placement/discovery/post-quest-priority/travel/3+boss combat/+25%/+15%/+10% Power retry/700-Gold-plus-ilvl10 Rare-or-Epic completion slice;
...
Persistent equipment contributes to both views. Ironwake Sentinel drops are generated as ilvl 1 `ItemInstance` objects: armor receives 5 inherent Armor, the sword receives 10 inherent Damage and +0.10 Attack Speed, and the shield receives 10 inherent Block. Ironward Vanguard remains ilvl 10 with 7 Armor, 13 sword Damage/+0.10 Attack Speed, and 13 shield Block. Each ilvl 10 necklace, earrings, or ring receives exactly one seeded inherent Fire/Cold/Lightning Resistance at value 20. These base stats remain the same across Common, Uncommon, and Rare. Serialized fixed stat fields on older visual definitions are not runtime stat sources.
