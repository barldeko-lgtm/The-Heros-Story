...
For the current Prototype 0 neutral quest choice, estimate recovery cost **relative to the weakest mob among quests that already passed Hard Filter**.

Let:

> `WeakestAllowedMobPower = minimum MobPower among quests that passed Hard Filter`

For each allowed quest:

> `RelativeRecoveryCost = MobPower / WeakestAllowedMobPower`

Therefore the weakest currently allowed mob has:

> `RelativeRecoveryCost = 1`

The current estimated cost of one ordinary mob is:

> **`EstimatedCostPerMob = 1 + RelativeRecoveryCost`**

where:

- the first `1` is the exactly one world tick consumed by the fight itself;
- `RelativeRecoveryCost` is the normalized estimate of post-fight recovery cost;
- the value may be fractional because this is a decision estimate, not actual world-tick execution.

The total mob part of a quest remains:

> `MobCount × EstimatedCostPerMob`

This heuristic deliberately uses the already shared `MobPower` rather than running a complete future combat simulation during quest evaluation. It may be retuned later as skills, equipment, buffs, and other combat systems make Power richer.
...
The current Prototype 0 estimate is resolved as:
...
The weakest allowed mob therefore has an estimated cost of:

> `1 fight tick + 1 recovery unit = 2`

Stronger allowed mobs scale proportionally from that weakest allowed mob.

The estimate is multiplied by `MobCount` inside `EstimatedQuestTicks`.

Fractional estimated ticks are allowed because the value exists only for comparing quests before selection. Actual combat and actual recovery still use their normal simulation rules.
...
The current Prototype 0 replacement is the normalized linear estimate:

> `EstimatedCostPerMob = 1 + (MobPower / WeakestAllowedMobPower)`

where `WeakestAllowedMobPower` is calculated only from quests that already passed Hard Filter.
