...
Protects the current Level 10 / 20 potion data, 1 / 2 / 3 / 4 Belt capacities, Belt-level potion eligibility, Belt comparison by total potential healing then inherent Health, full-slot preparation, healing-maximizing affordable mixed loadouts, multiple potion consumption in one between-fight window, allowed pre-boss overheal, mandatory full-Belt dungeon readiness, and the rule that a Belt purchase must not spend Gold needed to fill its resulting larger loadout.
...
Immutable healing-consumable data for the currently live Starting City tiers. Level 10 heals 100 HP for 100 Gold; Level 20 heals 150 HP for 200 Gold. These consumables are fixed shop availability rather than rotating equipment listings and currently have no dedicated inventory artwork/UI.
...
Performs candidate equipment evaluation without mutating live Equipment. Standard equipment resolves current/candidate base persistent CombatStats and recommends replacement only for a strict shared-HeroPower increase. Belt is the explicit utility exception: compare potential full-loadout potion healing first, then inherent Belt Health on an exact utility tie. Temporary effects remain excluded.
...
Owns the current dungeon healing-consumable preparation/use rules. It finds the highest-total-healing **complete** affordable loadout for the equipped or proposed Belt using shop potion definitions and existing Inventory potion counts, purchases only missing potions, records the prepared Belt levels on `HeroState`, and consumes physical Inventory counts during dungeon preparation windows. Ordinary-room use chooses the potion combination that restores the most HP without overheal, preferring fewer potions on an equal result; pre-boss use may consume multiple potions and accept overheal to reach full HP as closely as possible. It does not own dungeon travel/combat or equipment generation.
...
Contains the live ilvl 20 seven-piece Ironward Vanguard core family plus the existing ilvl 10 necklace, earrings, Ring 1, Ring 2, and Belt definitions in Common/Uncommon/Rare variants. The five core armor pieces keep their supplied visuals; core sword/shield retain neutral placeholders. The core family is connected to the five strongest ordinary mobs and the third Starting City shop band, but not to the first dungeon. The two mechanical ring slots share one supplied icon. Accessory icons appear only in equipment/inventory slots and provide no hero overlay. Belt contributes its inherent ilvl 10 Health, has no ordinary random affixes, and receives its live potion capacity from rarity plus potion-level eligibility from Item Level. Live inherent and random combat stats are generated on `ItemInstance`; old serialized experimental stat fields are ignored by current runtime generation.
