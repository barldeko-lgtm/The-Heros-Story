...
Immutable authored temporary-event content. `EventDefinition` owns placement/lifetime rules, optional secondary-objective placement constraints, and a small typed stage graph; stage/option resources own scene timing, Formative/Expressive decision data, `TRAVEL` targets, shared-combat references, outcomes, rewards, and future diary text. Current live content contains `0001_old_clearing_ambush.tres` (`У старой вырубки`) and `0002_smoke_over_old_tower.tres` (`Дым над старой башней`). The second event is the first to reserve a separate radius-0 objective and use real event-owned travel there and back.
...
Owns temporary-event definition loading, the global first-spawn gate at world tick 100, deterministic map placement, primary and optional secondary footprint reservation, active instances, encounter lookup, engagement, lifetime expiry, completion cleanup, and the current maximum-active-event cap. A secondary objective is selected only when all authored city-distance, event-distance, terrain/tag, non-overlap, and farther-from-origin constraints are satisfied; primary and secondary reservations are released together. During ticks 0–99 definitions are loaded/configured but no event footprint is placed; on tick 100 the initial population becomes eligible and uses the then-current free map space, retrying later if an already-active activity blocks every valid footprint. Pending initial events also receive placement priority before each later 50-tick quest-board refresh so available quest markers cannot permanently starve them of map space. It does not execute stages, move the hero, resolve combat, or mutate personality directly. Automatic replacement/population maintenance after an event disappears is not implemented yet.
...
### `tests/test_first_temporary_event_content.gd` / `test_first_temporary_event_runtime.gd` / `test_second_temporary_event_content.gd` / `test_second_temporary_event_runtime.gd` / `test_event_travel_suspend_resume.gd`
Protect both authored events, including their branch timings/rewards/personality meanings, exact shared combat references, secondary-objective placement, real event-owned travel, Expressive Brave/Greedy behavior, and suspension/resumption of the interrupted quest route.
