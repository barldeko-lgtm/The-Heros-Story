...
Owns real multi-tick map movement. It asks `HexMap` for the deterministic shortest route from the hero's current `WorldState.hero_position` to an already chosen destination, stores the active route, and advances exactly one adjacent hex per world tick when its caller advances travel. It can suspend an active destination and later rebuild the route from the hero's new position toward that original destination. `begin_detour()` replaces only the currently active route while preserving that suspended destination, allowing an engaged event to travel to its own objective and back without forgetting the interrupted quest. It exposes remaining route steps and arrival state but does not choose destinations, quests, events, or city relocation.
...
Immutable authored temporary-event content. `EventDefinition` owns placement/lifetime rules, optional secondary-objective placement constraints, and a small typed stage graph; stage/option resources own scene timing, Formative/Expressive decision data, `TRAVEL` targets, shared-combat references, outcomes, rewards, and future diary text. Current live content contains `0001_old_clearing_ambush.tres` (`У старой вырубки`), `0002_smoke_over_old_tower.tres` (`Дым над старой башней`), `0003_poachers_snares.tres` (`Чужие силки`), and `0004_dead_courier.tres` (`Мёртвый гонец`). The second event is the first to reserve a separate radius-0 objective and use real event-owned travel there and back; the third is the first to chain two distinct Expressive checks (`Curious` then `Noble`) after one Formative stat-driven opening; the fourth is the first detective-style event to make WIS / DEX / CON produce different safety/item/Gold consequences before later Devious and Curious expression.

### `scripts/model/runtime/event_instance.gd`
Mutable state for one spawned temporary event: primary target/reservation, optional secondary target/reservation, the actual encounter hex, spawn/expiry ticks, current stage, engagement/completion state, outcome id, and local flags. An engaged event no longer expires underneath the hero.

### `scripts/events/event_system.gd`
Owns temporary-event definition loading, the global first-spawn gate at world tick 100, the shared 200-tick population rotation, the current five-event cap, deterministic seeded pool selection/map placement, per-definition 500-tick engagement cooldowns, primary and optional secondary footprint reservation, active instances, encounter lookup, lifetime safety expiry, and completion cleanup. A secondary objective is selected only when all authored city-distance, event-distance, terrain/tag, non-overlap, and farther-from-origin constraints are satisfied; primary and secondary reservations are released together. During ticks 0–99 definitions are loaded/configured but no event footprint is placed. Tick 100 opens the first population; later rotations occur at ticks 300 / 500 / 700 / ... . A rotation removes every unengaged event from the previous cycle, preserves any event already being resolved, then selects up to the remaining five-slot capacity from definitions that are not active and not on cooldown. Selected definitions that cannot currently fit remain pending for that cycle and retry later; available quest-board markers yield placement priority at compatible 50-tick board refreshes, but an accepted active quest is never displaced. Engaging an event immediately starts its 500-tick definition cooldown; completion, expiry, or rotation without engagement does not create that cooldown. It does not execute stages, move the hero, resolve combat, or mutate personality directly.
...
Executes one engaged event graph. It records the real encounter hex, suspends the interrupted `TravelSystem` route, advances timed stages, executes real one-hex-per-tick `TRAVEL` stages toward the encounter or secondary objective through `TravelSystem.begin_detour()`, routes Formative movement through `TraitDevelopment`, reads established traits for Expressive checks, requests the shared event combat context, owns event-death resurrection/recovery state, and resumes the original travel destination after success. Event combat itself remains the shared `CombatSession`.
...
### `tests/test_first_temporary_event_content.gd` / `test_first_temporary_event_runtime.gd` / `test_second_temporary_event_content.gd` / `test_second_temporary_event_runtime.gd` / `test_third_temporary_event_content.gd` / `test_third_temporary_event_runtime.gd` / `test_fourth_temporary_event_content.gd` / `test_fourth_temporary_event_runtime.gd` / `test_event_population_rotation.gd` / `test_event_travel_suspend_resume.gd` / `test_event_dungeon_travel_interruption.gd`
Protect the four authored events plus the shared lifecycle: tick-100 opening, 200-tick population rotations, five-event cap, 500-tick engagement cooldown, pending selected definitions that cannot immediately fit, survival of an already-engaged event across a rotation, branch timings/rewards/personality meanings, exact shared combat references, secondary-objective placement, real event-owned travel, Expressive Brave/Greedy/Curious/Noble/Devious behavior, same-event Formative-to-Expressive trait activation, suspension/resumption of interrupted quest routes and outbound dungeon travel, and the rule that road-event death before dungeon entry cancels the trip without creating dungeon failure memory.
...
Owns one ordinary dungeon expedition after the city decision point. It accepts an already-discovered uncompleted dungeon after readiness has been approved, records HeroPower at the start of that expedition, starts the shared `TravelSystem` route to its real `target_hex`, owns the authored encounter cursor, carries current hero HP between fights, inserts exactly one world tick of preparation after every ordinary victory, advances into the dungeon's unique boss after its authored ordinary-room count, marks completion after boss victory, then starts and advances the real `TravelSystem` route back to the Starting City for the current Starting Region content. Outbound dungeon travel may be suspended by `EventRunner` and later resumed through the shared `TravelSystem`; completed-dungeon return travel is not yet event-interruptible. If an unrelated road event kills the hero before dungeon entry, `cancel_for_external_failure()` clears only the current expedition runtime and does not write dungeon failure memory. On successful arrival it releases the completed expedition and hands the hero back to the normal `VISITING_MARKET` city flow. It also owns dungeon-specific 100-tick death/resurrection + city-recovery state for deaths that actually occur inside dungeon combat. On dungeon combat failure it reports the attempt-start Power and reached progress so retry memory can be recorded. Potion selection/consumption does not live inside `DungeonRunner`: `Simulation` asks `PotionPreparationSystem` to apply approved healing inside the existing one-tick preparation window before advancing the runner to the next encounter. Combat itself remains the shared `CombatSession`; `QuestRunner` is not reused for dungeon execution.
