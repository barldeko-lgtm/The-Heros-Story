...
Also owns current `active_effects`, learned Power Strike and Battle Guard Skill Levels, the hero's `Equipment`, the current `Inventory`, and the ordered levels of healing potions currently prepared into Belt slots for dungeon use.
...
Immutable ordinary-dungeon data. Each ordinary dungeon authors one shared ordinary mob definition, a 3–5 ordinary-room count, and one unique boss; the same ordinary mob is therefore used in every pre-boss room of that dungeon. `Заброшенные железные шахты` uses a hill hex 4–7 steps from Starting City and `3 × Шахтный троглодит` → `Глубинный пожиратель`, rewarding 700 Gold + one ilvl 10 Rare/Epic item. `Городище Черноклыков` uses a forest hex 5–7 steps from Starting City and `3 × Гоблин-гвардеец` → `Король гоблинов`, rewarding 2000 Gold + one ilvl 20 Rare/Epic item from the full twelve-slot Ironward source. Both use `completion_epic_chance = 0.25`. Definitions store content/tuning only; they do not execute expeditions or award rewards.
...
Protects both current Starting Region dungeon definitions, including their fixed 3+boss structures, forest/hill placement constraints, approximately 200/300 and 600/750 Power tuning, XP values, absence of per-mob Gold/equipment drops, and ilvl 10/20 completion sources.
...
Protects the shared live combat loop across both current Starting Region dungeons: three ordinary fights, one preparation tick between encounters including before the boss, carried HP, boss completion, authored XP/Gold/item-level rewards, completed-map reservation removal, return travel, and dungeon-owned death/instant-resurrection behavior. The Blackfang path specifically protects its 1100 total combat XP, +2000 Gold, and one ilvl 20 Rare/Epic item.

### `tests/test_dungeon_completion_reward.gd`
Protects the exact 75% Rare / 25% Epic completion roll for both Starting Region dungeons, their full twelve-slot ilvl 10/20 source pools, normal three-affix Epic generation for standard equipment, and the affixless Belt exception whose rarity instead controls potion capacity.
...
Protects the current Level 10 / 20 potion data and supplied sprites, 1 / 2 / 3 / 4 Belt capacities, Belt-level potion eligibility, Belt comparison by total potential healing then inherent Health, full-slot preparation, healing-maximizing affordable mixed loadouts, one dedicated purchase tick when missing potions must be bought, multiple potion consumption in one between-fight window, allowed pre-boss overheal, mandatory full-Belt dungeon readiness, and the rule that a Belt purchase must not spend Gold needed to fill its resulting larger loadout.
...
Immutable healing-consumable data for the currently live Starting City tiers. Level 10 heals 100 HP for 100 Gold and uses `assets/items/icons/consumables/healing_potion_ilvl10.png`; Level 20 heals 150 HP for 200 Gold and uses `assets/items/icons/consumables/healing_potion_ilvl20.png`. Both supplied sources are 550 × 550. These consumables are fixed shop availability rather than rotating equipment listings.
...
Defines the Starting City shop and its three immutable authored stock bands: ilvl 1 Rustchain Initiate, ilvl 10 Ironwake core plus its existing accessories, and ilvl 20 Ironward Vanguard core equipment plus its own accessories. Each owns its source item definitions plus 6 White/2 Green listing counts. The same `ShopDefinition` separately references the fixed Level 10 / 20 healing-potion availability. Equipment bands do not duplicate generated stats, affix budgets, ItemPower, or price data.
...
The immutable drop tables store source item level, drop chance, and three aligned rarity pools. Five weaker mobs use the stable seven-slot `ironwake_sentinel_ilvl1_drop_table.tres` resource, whose content points to Rustchain Initiate. Giant Spider through Swamp Crocodile and the first dungeon completion reward use the twelve-slot `initial_equipment_drop_table.tres`, whose ilvl 10 content combines seven Ironwake core slots with five accessory slots. Young Ogre, Cave Lizard, Forest Troll, Mountain Beast, and Orc Raider use the twelve-slot `ironward_vanguard_ilvl20_drop_table.tres`, which combines seven Ironward core slots with five separate ilvl 20 accessory slots. The dungeon applies its own guaranteed Rare/Epic rarity override to the ilvl 10 source.

### `data/items/visual_families/ironward_vanguard/`
Contains the live ilvl 20 Ironward Vanguard core family plus separate necklace, earrings, Ring 1, Ring 2, and Belt definitions in Common/Uncommon/Rare variants, alongside the existing ilvl 10 accessory set. The five core armor pieces keep their supplied visuals; core sword/shield retain neutral placeholders. All twelve ilvl 20 slots are connected to the five strongest ordinary mobs and the third Starting City shop band, but not to the first dungeon. Each tier's two mechanical ring slots share one supplied icon. Accessory icons appear only in equipment/inventory slots and provide no hero overlay. Belts contribute inherent Health, have no ordinary random affixes, and receive live potion capacity from rarity plus potion-level eligibility from Item Level, so the new ilvl 20 Belt supports Level 20 bottles. Live inherent and random combat stats are generated on `ItemInstance`; old serialized experimental stat fields are ignored by current runtime generation.
...
Protects Rustchain across the unchanged ilvl 1 drops/shop, Ironwake across the seven core slots of the unchanged full ilvl 10 drops/shop/dungeon source, unchanged ilvl 10 accessories, the twelve-slot ilvl 20 Ironward source/shop band, and Rustchain inventory/paper-doll rendering. `test_ilvl20_accessories_and_belt.gd` narrowly protects the supplied ilvl 20 accessory icons, both ring slots, drop/shop connectivity, unchanged dungeon tier, and Level 20 potion support on the new Belt.
