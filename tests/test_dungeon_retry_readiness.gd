...
func _init() -> void:
	...
	print("PASS: Dungeon retry readiness uses +30% / +20% / +10% Power growth and blocks immediate repeat attempts after death.")
	quit()

func test_retry_thresholds() -> void:
	var simulation = SimulationScript.new(8301, null)
	var evaluator = simulation.dungeon_evaluator
	assert(is_equal_approx(evaluator.get_retry_growth(0, false), 0.30), "Dying before killing any ordinary dungeon enemy must require +30% HeroPower.")
	assert(is_equal_approx(evaluator.get_retry_growth(1, false), 0.20), "Killing at least one ordinary enemy but not reaching the boss must require +20% HeroPower.")
	assert(is_equal_approx(evaluator.get_retry_growth(2, false), 0.20), "Any ordinary-room progress short of the boss must keep the +20% HeroPower retry gate.")
	assert(is_equal_approx(evaluator.get_retry_growth(3, true), 0.10), "Reaching the boss and dying must require +10% HeroPower.")
	assert(is_equal_approx(evaluator.get_required_retry_power(100.0, 0, false), 130.0), "A 100-Power no-kill failure must require 130 Power.")
	assert(is_equal_approx(evaluator.get_required_retry_power(100.0, 1, false), 120.0), "A 100-Power ordinary-progress failure must require 120 Power.")
	...
	assert(not bool(evaluator.evaluate_retry_readiness(dungeon, 129.99)["ready"]), "A retry below the remembered required Power must stay blocked.")
	assert(bool(evaluator.evaluate_retry_readiness(dungeon, 130.0)["ready"]), "A retry at the remembered required Power must become valid.")
	dungeon.record_failed_attempt(130.0, 3, true)
	assert(dungeon.failed_attempt_count == 2, "Each failed retry must be remembered as another failed attempt.")
	assert(is_equal_approx(dungeon.last_failed_attempt_start_power, 130.0), "A later failure must replace the retry baseline with that attempt's own starting HeroPower.")
	assert(is_equal_approx(float(evaluator.evaluate_retry_readiness(dungeon, 143.0)["required_power"]), 143.0), "A boss failure on the later attempt must require 110% of that newer attempt baseline.")
...
func test_failed_first_fight_blocks_immediate_retry_until_power_growth() -> void:
	...
	assert(is_equal_approx(float(first_failure_readiness["retry_growth"]), 0.30), "First-fight death must resolve the approved +30% retry growth.")
	assert(is_equal_approx(float(first_failure_readiness["required_power"]), attempt_start_power * 1.30), "First-fight death must require 130% of attempt-start HeroPower.")
	assert(simulation.debug_log.get_text().contains("+30%"), "Failure log must explain the +30% Power retry requirement.")
