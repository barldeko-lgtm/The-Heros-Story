...
func _init() -> void:
	...
	assert(is_equal_approx(simulation.combat_stats.max_hp, 400.0), "Pending Constitution must provide no MaxHP before allocation.")
	assert(is_equal_approx(simulation.combat_stats.attack, 19.0), "Resolved physical Damage must refresh with only the fixed +1 Strength.")
	...
	assert(is_equal_approx(simulation.combat_stats.max_hp, 420.0), "Allocating Constitution through Simulation must immediately refresh resolved MaxHP.")
