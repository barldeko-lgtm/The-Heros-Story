...
func run_test() -> void:
	...
	assert(god_panel.vision_button.get_node("ActionDetail").text.contains("80"), "Vision button must display its 80-energy cost.")
	assert(main_ui.simulation.use_combat_buff(), "God UI test must activate the combat buff.")
	main_ui.update_hero_panel()
	god_panel.refresh()
	assert(god_panel.combat_buff_button.get_node("ActionDetail").text.contains("Боёв: 5"), "Active buff button must show remaining fights.")
	assert(god_panel.combat_buff_button.get_node("ActionDetail").text.contains("КД: 120"), "Active buff button must show cooldown counting from activation.")
	...
	assert(god_panel.instant_resurrection_button.get_node("ActionDetail").text.contains("10"), "Resurrection button must display its current dynamic energy cost.")
