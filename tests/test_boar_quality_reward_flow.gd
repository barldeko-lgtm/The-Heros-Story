...
func _init() -> void:
	...
	var common_result: Dictionary = simulation.receive_item_reward(common, 1, 20)
	...
	var duplicate_common_result: Dictionary = simulation.receive_item_reward(common, 2, 20)
	...
	var uncommon_result: Dictionary = simulation.receive_item_reward(uncommon, 3, 20)
	...
	var rare_result: Dictionary = simulation.receive_item_reward(rare, 4, 20)
	...
	assert(equipped_rare.item_level == 20 and equipped_rare.affixes.size() == 2, "Current Rare Ironward armor must be generated at ilvl 20 with two affixes.")
	...
	for reward_index in 33:
		last_result = simulation.receive_item_reward(common, 5 + reward_index, 20)
