...
func run_test() -> void:
	...
	assert(decoded["terrain_by_cell"].size() == map_definition.width * map_definition.height, "PNG must define every one of the 26 by 15 gameplay hexes.")
	...
	assert(map_definition.starting_city_center == Vector2i(6, 9), "Hero start must be decoded from the unique PNG marker.")
	assert(map_definition.mid_city_center == Vector2i(19, 5), "Mid-Level City center must be derived from its seven-hex PNG cluster.")
	...
	var recolored_cell := Vector2i(0, 5)
	...
	assert(str(invalid_result["errors"]).contains("(%d, %d)" % [recolored_cell.x, recolored_cell.y]), "Unknown-color error must report the affected hex coordinate.")
	...
	print("PASS: Exact-color PNG controls all 390 gameplay hexes, identifies hero start, rejects unknown colors, and feeds MapScreen.")
	quit()
