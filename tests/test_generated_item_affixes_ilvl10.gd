...
func _init() -> void:
	...
	assert(is_equal_approx(budget_table.get_green_affix_budget(5), 78.0), "ilvl 5 Green affix budget must preserve the former ilvl 10 value of 78.")
	...
	assert(is_equal_approx(base_stats.get_armor(5), 7.0), "ilvl 5 armor must preserve 7 base Armor.")
	assert(is_equal_approx(base_stats.get_sword_damage(5), 13.0), "ilvl 5 sword must preserve 13 base Damage.")
	assert(is_equal_approx(base_stats.get_shield_block(5), 13.0), "ilvl 5 shield must preserve 13 base Block.")

	var generator = generator_script.new(budget_table, stat_costs, base_stats)
	var common_helmet = generator.generate(load(COMMON_HELMET_PATH), 5, ScriptedRng.new())
	assert(common_helmet.item_level == 5 and common_helmet.rarity == 0, "Generated Common equipment must store the compressed ilvl and rarity on ItemInstance.")
	...
	assert(is_equal_approx(common_helmet.get_stat_bonus("armor"), 7.0), "Common ilvl 5 armor must keep the former ilvl 10 inherent 7 Armor.")
	...
	var uncommon_helmet = generator.generate(load(UNCOMMON_HELMET_PATH), 5, ScriptedRng.new([0.5], [0]))
	assert(uncommon_helmet.affixes.size() == 1, "Uncommon equipment must roll exactly one affix.")
	assert(is_equal_approx(uncommon_helmet.rolled_total_modifier_budget, 78.0), "A midpoint ilvl 5 Uncommon roll must keep the former nominal 78 budget.")
	...
	var rare_sword = generator.generate(load(RARE_SWORD_PATH), 5, ScriptedRng.new([0.5], [0, 0]))
	...
	assert(is_equal_approx(rare_sword.get_stat_bonus("attack_speed"), 0.10), "Every ilvl 5 sword rarity must retain the inherent +0.10 Attack Speed.")
	assert(rare_sword.get_item_power() > 0.0, "Generated base stats and affixes must contribute to ItemPower.")
	assert(rare_sword.get_tooltip_text().contains("Уровень предмета: 5"), "Generated-item tooltip must display the compressed Item Level.")
	...
	print("PASS: Shared generation preserves former ilvl 10 stats on compressed ilvl 5 items and source-driven ilvl 1 Goblin equipment.")
	quit()
