...
func _init() -> void:
	...
	assert(dropped_item != null and dropped_item.item_level == 10 and dropped_item.rarity == 1, "Every current mob drop must generate an ilvl 10 ItemInstance.")
	assert(drop_result["equipped"], "The first generated item for an empty slot must reach current equipment.")
	assert(is_equal_approx(simulation.base_combat_stats.armor, starting_armor + 7.0), "Generated inherent Armor must flow through Equipment and StatResolver.")
	assert(is_equal_approx(simulation.base_combat_stats.max_hp, starting_hp + 78.0 / 4.0), "Generated Health affix must flow through Equipment and StatResolver.")

	print("PASS: ilvl 10 drops generate inherent stats, rolled budgets, unique affixes, ItemPower, tooltip, and resolved hero stats.")
	quit()
