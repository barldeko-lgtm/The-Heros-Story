...
const JEWELRY_MOB_IDS := ["giant_spider", "bear", "mature_wolf", "rabid_elk", "bandit_veteran", "swamp_crocodile", "young_troll"]
...
func run_test() -> void:
	for slot in JEWELRY_FILES:
		for quality in 3:
			...
			if not ResourceLoader.exists(path):
				fail_test("Missing compressed ilvl 5 jewelry definition: %s" % path)
				return
	...
	if not ResourceLoader.exists(JEWELRY_DROP_TABLE_PATH):
		fail_test("Missing 12-slot compressed ilvl 5 equipment drop table.")
		return
	var jewelry_table: Resource = load(JEWELRY_DROP_TABLE_PATH)
	assert(jewelry_table.item_level == 5 and is_equal_approx(jewelry_table.drop_chance, 0.05), "Jewelry-enabled mob drops must use compressed ilvl 5 at the unchanged 5% drop chance.")
	for pool in [jewelry_table.common_items, jewelry_table.uncommon_items, jewelry_table.rare_items]:
		assert(pool.size() == 12, "The ilvl 5 source must roll all twelve equipment slots including Belt.")
	...
	assert(spider.equipment_drop_table == jewelry_table, "Giant Spider, the eighth mob by Power, must start the compressed ilvl 5 jewelry-enabled drop range.")
	assert(bandit.equipment_drop_table != jewelry_table and bandit.equipment_drop_table.item_level == 1, "The lower-band mobs must remain on seven-slot ilvl 1 drops.")
	var seen_weak: int = 0
	...
	for file_name in DirAccess.get_files_at("res://data/mobs"):
		...
		if WEAK_MOB_IDS.has(mob.id):
			...
			seen_jewelry += 1
			assert(mob.equipment_drop_table == jewelry_table, "Every middle-tier mob must use the shared twelve-slot ilvl 5 source.")
		else:
			seen_ilvl20 += 1
			assert(ILVL20_MOB_IDS.has(mob.id) and mob.equipment_drop_table.item_level == 10 and mob.equipment_drop_table.common_items.size() == 12, "Every strongest mob must use the separate twelve-slot compressed ilvl 10 source.")
	assert(seen_weak == 8 and seen_jewelry == 7 and seen_ilvl20 == 7, "The current roster must split into eight ilvl 1, seven jewelry-enabled ilvl 5, and seven jewelry-enabled ilvl 10 mobs.")
	...
	var generated_ring = generator.generate(common_ring, 5, ScriptedRng.new([], [1]))
	...
	assert(is_equal_approx(generated_ring.get_stat_bonus("cold_resistance"), 10.0), "The scripted ilvl 5 jewelry roll must use 10 percent Cold Resistance.")
	...
	assert(generated_ring.get_tooltip_text().contains("Базовое сопротивление холоду: +10.00%"), "Jewelry tooltip must show its inherent elemental Resistance as a direct percentage.")

	var uncommon_necklace: Resource = load("%s/ironward_necklace_uncommon.tres" % JEWELRY_DIRECTORY)
	var generated_necklace = generator.generate(uncommon_necklace, 5, ScriptedRng.new([0.5], [0, 3]))
	...
	assert(is_equal_approx(generated_necklace.get_stat_bonus("max_hp"), 78.0 / 4.0), "Jewelry Health affixes must preserve the former ilvl 10 budget at compressed ilvl 5.")
	...
	assert(common_belt != null and uncommon_belt != null and rare_belt != null, "The ilvl 5 Belt must have Common, Uncommon, and Rare definitions.")
	...
	var generated_belt = generator.generate(uncommon_belt, 5, ScriptedRng.new())
	assert(generated_belt != null and is_equal_approx(generated_belt.get_stat_bonus("max_hp"), 40.0), "The compressed ilvl 5 Belt must preserve base Health 40.")
	...
	assert(belt_rules.get_capacity(generated_belt) == 2 and is_equal_approx(belt_rules.get_potential_healing(generated_belt), 200.0), "Uncommon ilvl 5 Belt must provide two 100-HP potion slots.")
	...
	assert(equipped_ring != null and equipped_ring.item_level == 5 and equipped_ring.definition.equipment_slot == "ring_1", "A Spider jewelry roll must create and route a real compressed ilvl 5 ring.")
	assert(reward.get("equipped", false), "The first ring must equip through ordinary virtual-equip routing.")
	assert(is_equal_approx(simulation.base_combat_stats.lightning_resistance, 20.0), "Equipped ilvl 5 jewelry Resistance must add 10 percentage points on top of the Warrior's innate 10 percent baseline.")
	...
	assert(is_equal_approx(equipped_belt.get_stat_bonus("max_hp"), 40.0), "Equipped ilvl 5 Belt must contribute the unchanged 40 Health through the normal stat pipeline.")
	...
	print("PASS: Compressed ilvl 5 jewelry uses the approved Resistance tuning and Belt keeps all twelve equipment slots through shared paths.")
	quit()
