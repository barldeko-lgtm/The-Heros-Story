...
func run_test() -> void:
	...
	assert(generated_ring != null and generated_ring.base_stats.size() == 1, "Common jewelry must generate exactly one inherent elemental Resistance.")
		assert(is_equal_approx(generated_ring.get_stat_bonus("cold_resistance"), 20.0), "The scripted ilvl 5 jewelry roll must preserve 20 Cold Resistance.")
	...
	inventory_screen.free()
		print("PASS: Compressed ilvl 5 jewelry and Belt preserve their old stats and use all twelve equipment slots through shared paths.")
	quit()
