...
func test_shield_bash_priority_stun_and_fallback() -> void:
	...
	assert(session.rage == 85, "Shield Bash must cost exactly 15 Rage.")
	var expected_base_wisdom_stun := 5.0 + 2.0 * (5.0 / 105.0)
	assert(is_equal_approx(session.mob_next_attack_time, 2.0 + expected_base_wisdom_stun), "SL1 Shield Bash must start from 5 seconds at 0 WIS and add specialization WIS scaling from total WIS.")
	...
	var zero_wis = CombatSessionScript.new(hero, mob, make_rng(12), 1.0, 0, 0, 0, 1, 0, true)
	assert(is_equal_approx(zero_wis.get_shield_bash_stun_duration(), 5.0), "Shield Bash SL1 must be exactly 5 seconds at 0 WIS.")
	var wis_40 = CombatSessionScript.new(hero, mob, make_rng(13), 1.0, 0, 40, 0, 1, 0, true)
	assert(is_equal_approx(wis_40.get_shield_bash_stun_duration(), 5.0 + 2.0 * (40.0 / 140.0)), "Shield Bash must scale from total WIS, including WIS 40.")
	var max_rank = CombatSessionScript.new(hero, mob, make_rng(14), 1.0, 0, 0, 0, 10, 0, true)
	assert(is_equal_approx(max_rank.get_shield_bash_stun_duration(), 7.0), "Shield Bash SL10 must reach 7 seconds at 0 WIS.")
...
func test_crippling_blows_priority_damage_and_slow() -> void:
	...
	var expected_damage_multiplier := 0.75 + 0.03 * (5.0 / 105.0)
	for action in actions:
		...
		assert(is_equal_approx(action.damage, 100.0 * expected_damage_multiplier), "Each Crippling Blows strike must use the approved rank- and WIS-scaled weapon-hit coefficient.")
	assert(session.rage == 85, "Crippling Blows must cost exactly 15 Rage.")
	var expected_base_wisdom_reduction := 0.25 + 0.15 * (5.0 / 105.0)
	assert(is_equal_approx(session.crippling_slow_reduction, expected_base_wisdom_reduction), "SL1 Crippling Blows must start from 25 percent at 0 WIS and add specialization WIS scaling from total WIS.")
	...
	var expected_slowed_interval := 2.0 / (1.0 - expected_base_wisdom_reduction)
	...
	var zero_wis = CombatSessionScript.new(hero, mob, make_rng(21), 1.0, 0, 0, 0, 0, 1, true)
	assert(is_equal_approx(zero_wis.get_crippling_blows_attack_speed_reduction(), 0.25), "Crippling Blows SL1 must be exactly 25 percent at 0 WIS.")
	var wis_80 = CombatSessionScript.new(hero, mob, make_rng(22), 1.0, 0, 80, 0, 0, 1, true)
	assert(is_equal_approx(wis_80.get_crippling_blows_attack_speed_reduction(), 0.25 + 0.15 * (80.0 / 180.0)), "Crippling Blows must scale from total WIS, including WIS 80.")
	var max_rank = CombatSessionScript.new(hero, mob, make_rng(23), 1.0, 0, 0, 0, 0, 10, true)
	assert(is_equal_approx(max_rank.get_crippling_blows_attack_speed_reduction(), 0.40), "Crippling Blows SL10 must reach 40 percent at 0 WIS.")
	assert(is_equal_approx(zero_wis.get_crippling_blows_damage_multiplier(), 0.75), "Crippling Blows SL1 must use exactly a 0.75 damage coefficient at 0 WIS.")
	assert(is_equal_approx(max_rank.get_crippling_blows_damage_multiplier(), 0.795), "Crippling Blows SL10 must gain 0.005 damage coefficient per additional Skill Level at 0 WIS.")
	assert(is_equal_approx(wis_80.get_crippling_blows_damage_multiplier(), 0.75 + 0.03 * (80.0 / 180.0)), "Crippling Blows damage must receive the approved small total-WIS coefficient bonus.")
...
func test_attack_progress_rescaling() -> void:
	...
	session.apply_crippling_slow(0.25)
	assert(is_equal_approx(session.mob_next_attack_time, 2.5), "Slow application must preserve 25 percent completed attack progress.")
	...
	assert(is_equal_approx(session.mob_next_attack_time, 2.125), "Slow removal must preserve the current percentage of attack progress instead of resetting or granting free progress.")
