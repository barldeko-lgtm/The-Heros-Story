...
func _init() -> void:
	...
	assert(is_equal_approx(starting_stats.crit_chance, 0.125), "5 DEX must add 2.5 percentage points of Critical Chance.")
	assert(is_equal_approx(starting_stats.crit_damage, 1.55), "5 STR must add 5 percentage points of Critical Damage.")
	...
	assert(is_equal_approx(level_two_stats.crit_chance, 0.125), "Unspent DEX must not change Critical Chance.")
	assert(is_equal_approx(level_two_stats.crit_damage, 1.56), "Level 2 Critical Damage must include only 6 STR before player allocation.")
