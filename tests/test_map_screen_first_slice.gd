...
func run_test() -> void:
	...
	assert(not spawned_events.is_empty(), "MapScreen integration test must be able to open the first event population at world tick 100.")
	...
	var event_instance = null
	for candidate in active_events:
		if candidate != null and candidate.definition != null and candidate.definition.id == "old_clearing_ambush":
			event_instance = candidate
			break
	assert(event_instance != null, "Old Clearing must remain present for the existing radius-1 map-shading regression check.")
