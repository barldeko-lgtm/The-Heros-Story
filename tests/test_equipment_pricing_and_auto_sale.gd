...
func _init() -> void:
	...
	assert(price_calculator.get_reference_shop_value(10, 0) == 500, "ilvl 10 White reference shop value must be 500 Gold.")
	assert(price_calculator.get_reference_shop_value(10, 1) == 1500, "ilvl 10 Green reference shop value must be 1500 Gold.")
	assert(price_calculator.get_reference_shop_value(10, 2) == 4500, "ilvl 10 Rare reference shop value must be 4500 Gold.")
	assert(price_calculator.get_sell_price_for_level_and_rarity(10, 0) == 50, "ilvl 10 White sell value must be 50 Gold.")
	assert(price_calculator.get_sell_price_for_level_and_rarity(10, 1) == 150, "ilvl 10 Green sell value must be 150 Gold.")
	assert(price_calculator.get_sell_price_for_level_and_rarity(10, 2) == 450, "ilvl 10 Rare sell value must be 450 Gold.")
	...
	var common_item = simulation.item_generator.generate(load(COMMON_CHEST_PATH), 10, ScriptedRng.new())
	var uncommon_item = simulation.item_generator.generate(load(UNCOMMON_HELMET_PATH), 10, ScriptedRng.new([0.5], [0]))
	var rare_item = simulation.item_generator.generate(load(RARE_SWORD_PATH), 10, ScriptedRng.new([0.5], [0, 0]))
	...
	assert(sale_result["gold_gained"] == 650, "White + Green + Rare ilvl 10 resale must grant 50 + 150 + 450 Gold.")
