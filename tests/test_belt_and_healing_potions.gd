...
func _init() -> void:
	...
	print("PASS: Belt utility, ilvl 5/10 healing potions, full-slot preparation, and dungeon-only between-fight healing follow the approved rules.")
	quit()
...
func test_potion_data_and_belt_comparison() -> void:
	...
	var common10 = simulation.item_generator.generate(common_definition, 5, EmptyRng.new())
	var uncommon10 = simulation.item_generator.generate(uncommon_definition, 5, EmptyRng.new())
	var rare10 = simulation.item_generator.generate(rare_definition, 5, EmptyRng.new())
	var common20 = simulation.item_generator.generate(common_definition, 10, EmptyRng.new())
	var uncommon20 = simulation.item_generator.generate(uncommon_definition, 10, EmptyRng.new())
	...
	assert(is_equal_approx(rules.get_potential_healing(rare10), 300.0), "Blue ilvl 5 Belt must represent 3 x 100 = 300 potential healing.")
	assert(is_equal_approx(rules.get_potential_healing(common20), 150.0), "White ilvl 10 Belt must represent 1 x 150 = 150 potential healing.")
	...
	assert(not bool(weaker_utility["should_equip"]), "White ilvl 10 Belt must not replace Blue ilvl 5 when its total potion healing is lower, despite higher Belt Health.")
	var equal_utility: Dictionary = simulation.equipment_evaluator.evaluate(simulation.hero_state, uncommon20)
	assert(bool(equal_utility["should_equip"]), "Green ilvl 10 Belt must replace Blue ilvl 5 when both provide 300 potion healing and ilvl 10 has more inherent Health.")
...
func test_full_preparation_and_between_fight_use() -> void:
	...
	var uncommon_definition = load("%s/ironward_belt_uncommon.tres" % BELT_DIR)
	var belt20 = simulation.item_generator.generate(uncommon_definition, 10, EmptyRng.new())
	...
	assert(int(plan["purchase_cost"]) == 300 and is_equal_approx(float(plan["total_healing"]), 250.0), "With 300 Gold, a two-slot ilvl 10 Belt must maximize healing with one ilvl 10 and one ilvl 5 potion.")
	...
	simulation.hero_state.prepared_healing_potion_levels = [10, 5, 5]
	...
	assert(simulation.hero_state.inventory.get_healing_potion_count(10) == 1, "The stronger 150-HP potion must be preserved when two weaker potions produce an exact no-overheal ordinary-room heal.")

	simulation.hero_state.inventory.add_healing_potion(5)
	simulation.hero_state.prepared_healing_potion_levels = [5, 10]
...
func test_dungeon_requires_full_belt() -> void:
	...
	assert(simulation.hero_state.gold == 0 and simulation.hero_state.prepared_healing_potion_levels == [5], "The dedicated potion tick must buy and prepare the full one-slot ilvl 5 loadout for exactly 100 Gold.")
	assert(simulation.hero_state.inventory.get_healing_potion_count(5) == 1, "Prepared dungeon potion must still exist in the hero Inventory until consumed.")
...
func test_belt_purchase_preserves_new_full_loadout_budget() -> void:
	...
	# Green ilvl 5 Belt costs 1500 Gold and would raise capacity from one to two slots.
	...
	assert(simulation.hero_state.gold == 1500 and simulation.hero_state.prepared_healing_potion_levels == [5], "The dedicated potion tick must spend exactly 100 Gold on the current one-slot Belt preparation.")
